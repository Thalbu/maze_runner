...
bool resolver_labirinto(vector<vector<char>>& labirinto, int linhas, int colunas, pair<int, int> inicio) {
    ...
    while (!pilha.empty()) {
        ...
        if (caminhos_validos.size() > 1) {
            // Cria threads adicionais para os caminhos extras
            ...
            // Continua a explorar o primeiro caminho com a thread atual
            pilha.push(caminhos_validos[0]);

            // Espera todas as threads adicionais finalizarem
            for (auto& t : threads) {
                t.join();
            }
